...
# read in the data
species_file <- read_csv(here::here("data","raw","ASDN_Daily_species.csv"))

snowsurvey_file <- read_csv(here::here("data","raw","ASDN_Snow_survey.csv"))
```
...
# Import the species
# snowsurvey_fil <- read_csv(file.path(datadir_raw, snowsurvey_file))

glimpse(snowsurvey_file)
...
```{r}
snowsurvey_fixed <- snow_survey_fixed %>% 
...
```{r}
snowsurvey_fixed <- snowsurvey_file %>% 
...
```{r}
write_csv(snowsurvey_fixed, here::here("data","processed", "snow_cover.csv"))
```
...
species_presence <- species_long %>% 
  mutate(species_presence = ifelse())
```
